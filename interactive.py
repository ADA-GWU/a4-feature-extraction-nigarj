...
class ContourDrawer:
    ...
    def draw_instructions(self, frame):
        ...
        h, w = frame.shape[:2]
        instructions = [
            "Draw initial contour by dragging with left mouse",
            "Press SPACE to run active contour",
            "Press R to reset, or close window to exit"
        ]
        for i, text in enumerate(instructions):
            cv2.putText(overlay, text, (10, h - 60 + i * 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        return overlay

    def run(self):
        print("[i] Draw initial contour. Press SPACE to process, R to reset, or close window to exit.")
        while True:
